...
public class LoadBalancer {
    private static void startLoadBalancer(String schedAlgo) {
        try {
            ...
            BufferedReader workerFile = new BufferedReader(new FileReader(new File("src/Dependency/worker_list.txt")));
            ...
            while (!Thread.interrupted()) {
                ...
                else if(schedAlgo.equals("HR"))
                {
                    currentWorker = workerLoads.getMaxHealthServer();
                    double health=workerLoads.getHealth(currentWorker);
                    System.out.println("Selected worker " + currentWorker + " with Health factor: " + health + ".");
                    workerLoads.incrementLoad(currentWorker);
                }
